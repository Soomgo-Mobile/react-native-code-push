...
| `--only android\|ios` | 한 플랫폼만 실행. 지정하지 않으면 한 번의 E2E 실행에서 두 플랫폼이 나란히 돌며, 시뮬레이터와 에뮬레이터(또는 연결된 기기)가 동시에 필요합니다 | both |
...
4. iOS 빌드 전 `RCT_NEW_ARCH_ENABLED=0 npm run setup:pods`를 실행한 뒤, `RCT_NEW_ARCH_ENABLED=0` 환경으로 E2E 실행
...
- RN `0.81.x`의 legacy iOS 실행에서도 각 앱의 `setup:pods`가 prebuild opt-in 플래그를 유지합니다.

...
- **Diff Updates** – Publish [binary patch and asset diff releases](docs/diff-updates.md) so a client downloads the difference from what it already holds, not the whole bundle.
...
plus an optional fourth one - `bundleDownloader` - if you publish [asset diff archives](docs/diff-updates.md#asset-diff-archives).
...
bundle inside the app binary. A release that carries one can also carry an asset diff, for a
client running one of the recent updates the release was diffed against. Both need a build
hook that copies the JS bundle out of your store build; asset diffs also need a
`bundleDownloader` in `code-push.config.ts`.

See [Diff updates](docs/diff-updates.md) for the hooks, what to keep per binary version, and
how a client picks an archive.
